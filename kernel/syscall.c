...
sys_uname(void) {
  ...
  struct utsname info;
  strncpy(info.sysname, "OS", 3);
  strncpy(info.nodename, "No Blues", 9);
  strncpy(info.release, "release", 8);
  strncpy(info.version, "0.1", 4);
  strncpy(info.machine, "test", 5);
  strncpy(info.domainname, "uname", 5);
